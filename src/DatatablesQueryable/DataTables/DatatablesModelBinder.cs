using Microsoft.AspNetCore.Mvc.ModelBinding;
using Microsoft.AspNetCore.WebUtilities;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace DatatablesQueryable.DataTables
{
    public class DataTablesModelBinder : IModelBinder
    {
        public Task BindModelAsync(ModelBindingContext bindingContext)
        {
            if (bindingContext.ModelType == typeof(DTParameterModel))
            {
                var queryString = bindingContext.HttpContext.Request.QueryString.Value;
                var decoded = System.Net.WebUtility.UrlDecode(queryString);
                var request = QueryHelpers.ParseQuery(decoded);

                // Retrieve request data
                var draw = Convert.ToInt32(request["draw"]);
                var start = Convert.ToInt32(request["start"]);
                var length = Convert.ToInt32(request["length"]);

                // Search
                var search = new DTSearch
                {
                    Value = request["search[value]"],
                    Regex = Convert.ToBoolean(request["search[regex]"])
                };

                // Order
                var o = 0;
                var order = new List<DTOrder>();
                while (request.ContainsKey("order[" + o + "][column]"))
                {
                    order.Add(new DTOrder
                    {
                        Column = Convert.ToInt32(request["order[" + o + "][column]"]),
                        Dir = request["order[" + o + "][dir]"]
                    });
                    o++;
                }

                // Columns
                var c = 0;
                var columns = new List<DTColumn>();
                while (request.ContainsKey("columns[" + c + "][name]"))
                {
                    columns.Add(new DTColumn
                    {
                        Data = request["columns[" + c + "][data]"],
                        Name = request["columns[" + c + "][name]"],
                        Orderable = Convert.ToBoolean(request["columns[" + c + "][orderable]"]),
                        Searchable = Convert.ToBoolean(request["columns[" + c + "][searchable]"]),
                        Search = new DTSearch
                        {
                            Value = request["columns[" + c + "][search][value]"],
                            Regex = Convert.ToBoolean(request["columns[" + c + "][search][regex]"])
                        }
                    });
                    c++;
                }

                var result = ModelBindingResult.Success(
                    new DTParameterModel
                    {
                        Draw = draw,
                        Length = length,
                        Start = start,
                        Search = search,
                        Order = order,
                        Columns = columns
                    });
                bindingContext.Result = result;
            }

            return Task.FromResult(0);
        }
    }
}
